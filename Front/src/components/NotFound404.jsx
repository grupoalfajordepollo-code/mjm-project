...
const NotFound404 = () => {
  ...
  // Un admin perdido vuelve al panel, el resto al home
  const esAdmin = isAuthenticated && rol === "admin";
  const destino = esAdmin ? "/panel-secured" : "/";

  return (
    <main className="w-full min-h-screen flex flex-col items-center justify-center bg-gray-50 font-space p-6">
      ...
      <div className="max-w-lg w-full bg-white rounded-3xl shadow-[0_8px_30px_-4px_rgba(0,0,0,0.05)] border border-[#e6d5cc] p-8 md:p-12 text-center relative overflow-hidden">
        ...
        <h1 className="text-5xl font-black text-[#B02F00] mb-2 tracking-tight">404</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Link
            to={destino}
            className="cursor-pointer flex items-center justify-center gap-2 px-6 py-3.5 bg-[#B02F00] hover:bg-[#8a2500] text-white text-sm font-bold rounded-lg transition-colors"
          >
            {esAdmin ? <LayoutDashboard size={18} strokeWidth={2} /> : <Home size={18} strokeWidth={2} />}
